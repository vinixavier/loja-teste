...
function AdicionarProdutoNoCarrinho(idProduto, nomeProduto, precoProduto, quantidadeProduto) {
    ...
    const posicaoDoProdutoExistente = produtosAdicionadosNoCarrinho.findIndex((produtoDentroDoArray) => produtoDentroDoArray.idProduto === idProduto)

    if (posicaoDoProdutoExistente == -1) {
        produtosAdicionadosNoCarrinho.push(novoProduto)
    } else {
        produtosAdicionadosNoCarrinho[posicaoDoProdutoExistente].quantidadeProduto += quantidadeProdutoEmNumber;
    }
...
function escreveProdutosNaListaDeCompras(listaDeProdutosDentroDoCarrinho) {
    ...
    listaDeProdutosDentroDoCarrinho.forEach(function (produto) {
        ...
        li.innerText = `${produto.nomeProduto} - (${produto.quantidadeProduto})x ${produto.precoProduto} - SubTotal: ${floatParaMoeda(produto.precoProduto * produto.quantidadeProduto)}`
...
function calcularTotalProdutos() {
    ...
    for (let posicao = 0; posicao < produtosAdicionadosNoCarrinho.length; posicao++) {
        totalProdutos += produtosAdicionadosNoCarrinho[posicao].precoProduto * produtosAdicionadosNoCarrinho[posicao].quantidadeProduto;
    }
